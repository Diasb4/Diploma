    ...
    <Modal title="Вход в AITU Diploma" description="Используйте корпоративную почту университета. Для демонстрации поле уже заполнено." onClose={() => setIsAuthModalOpen(false)} width="md">
      <form onSubmit={submit} className="space-y-4">
        <label className="block text-sm font-medium">Почта AITU
          <input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm focus:border-[#0b5fff] focus:outline-none dark:border-slate-700 dark:bg-[#0f131a]" placeholder="student@astanait.edu.kz" />
        </label>
